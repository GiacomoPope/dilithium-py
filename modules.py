class Module:
    ...
    def __bit_unpack(
        self, input_bytes, m, n, alg, packed_len, *args, is_ntt=False
    ):
        poly_bytes = [
        ...
        matrix = [
            [
                alg(poly_bytes[n * i + j], *args, is_ntt=is_ntt)
                for j in range(n)
            ]
            for i in range(m)
        ]
    ...
    def bit_unpack_t0(self, input_bytes, m, n, is_ntt=False):
        ...
        return self.__bit_unpack(
            input_bytes, m, n, algorithm, packed_len, is_ntt=False
        )
    ...
    class Matrix:
        ...
        def __bit_pack(self, algorithm, *args):
            return b"".join(
                algorithm(poly, *args) for row in self.rows for poly in row
            )
        ...
        def copy_from_ntt(self):
            matrix = [
                [ele.copy_from_ntt() for ele in row] for row in self.rows
            ]
            return self.parent(matrix)
        ...
        def __repr__(self):
            ...
            for n_col in range(self.n):
                max_col_width.append(
                    max(len(str(row[n_col])) for row in self.rows)
                )
            info = "]\n[".join(
                [
                    ", ".join(
